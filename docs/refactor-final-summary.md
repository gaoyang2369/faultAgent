...
本文档汇总 Phase 1 到 Phase 4.4R 的整体改造结果，用于替代分散的阶段审计说明，并记录当前真实执行边界。
...
- Phase 4.4R：新增 high-risk workorder/action dry-run readiness、manual confirmation contract，以及 legacy `TaskType` / `intent_stack` 删除前依赖扫描。
...
`ResolvedContext`、`GoalSet`、`TaskFamily`、`ShadowPlanner`、`PlanningDiff`、`DiagnosisReadiness`、`WorkorderActionReadiness` 和 `ManualConfirmationRequirement` 都是增量的观测、调试和 eval 层。只有 `PlannerGate` 允许影响执行，而且必须显式开启并处于 active 模式。默认仍是 legacy execution；显式开启后，4.3 只读任务可收窄到 `knowledge` / `sql` / `report`，4.4.2 只允许满足严格 readiness 的 `alarm_triage` / `fault_diagnosis` 进入 explanation-only limited active。Phase 4.4R 不允许 action/workorder active。
...
Phase 4.4R 没有任何环境变量允许 action/workorder 迁移执行。diagnosis active 只限 explanation-only scope；workorder/action 只允许 dry-run readiness 和人工确认合同，不允许工单派发、设备动作、参数修改或已执行语义。
...
| 4.4R | 增加 high-risk dry-run 和 legacy 删除 readiness。 | `planning/action_readiness.py`、`planning/manual_confirmation.py`、`scripts/legacy_dependency_scan.py`。 | `workorder_action_readiness`、`manual_confirmation`、legacy dependency scan reports。 | 不影响。action/workorder 永远 `legacy_policy`，只输出 dry-run 和人工确认合同。 | `scripts/high_risk_dry_run_acceptance_test.py`：8 passed；dependency scan generated。 |
...
`WorkorderActionReadiness` 回答的问题是：高风险 workorder/action 请求在删除 legacy 字段或进入 active 前还缺什么？
...
`workorder_decision` 和 `workorder_draft` 只能进入 dry-run 观测；`device_action` 永远保持 `keep_legacy` / blocked。stale evidence 必须 refresh 或 disclosure；无权限、歧义、缺设备、缺证据、critical diff、needs_review 都会继续 fallback legacy。

### ManualConfirmationRequirement

`ManualConfirmationRequirement` 回答的问题是：如果用户要求工单或动作，下一步最多允许到哪里？
...
不允许 dispatch/execute/reset/stop/parameter-change。final answer 和 output plan 不得出现“已派发 / 已执行 / 已复位 / 已停机 / 已修改参数”等完成式动作语义。

### Legacy Deletion Readiness

`scripts/legacy_dependency_scan.py` 会扫描 `TaskType` / `primary_task_type` / `candidate_task_types` / `intent_stack` 的读写依赖，并输出：

- `trash/run/legacy_dependency_scan.json`
- `trash/run/legacy_dependency_scan.md`

当前结论是短期不能删除 `TaskType` / `intent_stack`。它们仍被 workflow policy、planner diff、gate、tests/evals、SSE payload 和 artifact-compatible schema 消费。安全路线是先 deprecate，再 compatibility-only，最后 remove internal dependency；短期不能从 SSE / artifact 删除字段。
...
- 顶层 `workorder_action_readiness`
- 顶层 `manual_confirmation`
...
- `workflow_route.manual_confirmation`
...
- `PYTHONPATH=. pytest -q`：238 passed。
...
- `PYTHONPATH=. python scripts/high_risk_dry_run_acceptance_test.py`：8 passed。
...
- `PYTHONPATH=. python scripts/legacy_dependency_scan.py`：scan generated；TaskType read files 39，intent_stack read files 23。
...
Phase 4.4R 已完成 high-risk dry-run 和 legacy deletion readiness 的最小闭环。下一阶段可以进入 legacy deprecation phase，但不能删除 `TaskType` / `intent_stack`，也不能开始 action/workorder active migration。

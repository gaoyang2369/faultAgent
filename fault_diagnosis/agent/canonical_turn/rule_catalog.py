...
RULE_CATALOG = (
    ...
    },
    {
        ...
        "pattern": r"(?:判断|评估|看看|问问|考虑|知道|想知道)?.{0,14}(?:是否|要不要|有没有|有无|是否需要|是否建议|有没有必要|有必要).{0,14}(?:工单|维修单|报修|建单|安排.{0,4}(?:工程师|人员?).{0,4}处理)|(?:工单|维修单|报修|建单).{0,10}(?:是否需要|有没有必要|有必要|必要性|建议)|(?:评估|判断).{0,8}(?:工单)?必要性|考虑.{0,8}(?:工单|报修)|(?:需要|建议).{0,6}(?:生成|创建|新建)?(?:工单|维修单|报修|建单)(?:吗|么|[?])",
    ...
    },
    {
        ...
        "category": "action_predicate",
        "pattern": r"(?:诊断(?!结果)(?:一下|看看)?|判断|排查|分析).{0,24}(?:故障|异常|问题|原因|根因|[A-Za-z]{1,3}\d{4,6})|(?:运行数据|运行状态).{0,12}(?:判断|诊断|分析)|(?:基于|根据|使用|用).{0,16}(?:结果|数据).{0,6}(?:分析|诊断)|(?:详细)?诊断(?!结果)(?:一下|看看)?|继续分析|(?:为什么|为何).{0,18}(?:故障|异常|问题)|(?:有没有|是否|存在|是不是).{0,8}(?:故障|异常|问题|不太对劲|毛病)|(?:故障|异常)吗|(?:如果|若).{0,12}(?:故障|异常|问题)$|(?:不用|无需|不必|不要).{0,5}(?:重新)?诊断",
    ...
    },
    {
        ...
        "category": "action_predicate",
        "pattern": r"(?<!排)(?:查询|查看|检查|看看|查|改查|改成(?:看|查)?|展示|告诉我).{0,24}(?:状态|运行|异常|数据|电机\d+)|查完.{0,16}(?:J\d+|电机\d+)|(?:当前|现在|最近).{0,18}(?:状态|运行|异常|健康|咋样)|(?:健康状况|运行情况|最近咋样)|(?:不用|无需|不必|不要).{0,8}(?:重新)?查询",

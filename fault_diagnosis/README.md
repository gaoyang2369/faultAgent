...
- Grounded answer（默认关闭）：`ENABLE_GROUNDED_ANSWER_SYNTHESIS=false`、`ANSWER_SYNTHESIS_TIMEOUT_SECONDS=20`、`ANSWER_SYNTHESIS_MAX_INPUT_CHARS=12000`、`ANSWER_SYNTHESIS_MAX_OUTPUT_CHARS=4000`、`ANSWER_SYNTHESIS_MAX_OUTPUT_TOKENS=1000`、`ANSWER_SYNTHESIS_TEMPERATURE=0.0`、`ANSWER_SYNTHESIS_MAX_CONCURRENCY=8`。启用后仅对 V2 已完成的结构化交付物做一次低温度 JSON 措辞生成；模型异常或确定性校验失败时继续使用 `CompositePresenter` 原回答。

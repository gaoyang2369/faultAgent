...
def scenario_report_then_workorder(client: TestClient) -> None:
    ...
    assert_true(snapshot["requested_output"] == "action_confirmation", "B must expect action confirmation")
...
def scenario_explicit_device_switch(client: TestClient) -> None:
    ...
    assert_true(snapshot["enabled_nodes"].get("sql") is True, "E must enable SQL")
